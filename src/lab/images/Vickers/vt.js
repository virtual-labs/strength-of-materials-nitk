...
function magic()
{
	...
	if(simsubscreennum==6)
	{
		...
		$('#6-6').mousemove(function(){
			...
			elem.style.left=parseFloat(elem.style.left)+0.2+'px';
			var pos=$(elem).offset();
			if(pos.left==56)
			{
				myStopFunction();
	...
	if(simsubscreennum==7)
	{
		...
		myInt=setInterval(function(){animateArrow();},500);
		document.getElementById('arrow1').style="position:absolute; visibility:visible; left:200px; top:100px; height:30px; z-index:10";
		document.getElementById('arrow1').WebkitTransform="rotate(0deg)";
		// Syntax for IE9
		document.getElementById('arrow1').msTransform="rotate(0deg)";
		// Standard syntax
		...
		$('#7-6').mousemove(function(){
			...
			elem1.style.top=parseFloat(elem1.style.top)+-0.2+'px';
